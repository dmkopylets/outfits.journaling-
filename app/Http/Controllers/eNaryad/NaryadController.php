...
use App\Models\eNaryad\Dicts\Station_Type;
...
{          
     ...
     !! і передаємо у session для наступних форм введення */
    {
        ...
        if (isset($record->brigade_m)) {
            $brigade_txt = Brigade_Member::find(explode(",",$record->brigade_m));
        }
        $engineers_txt='';
    ...
    public function store(Request $request)  {        
                ...
                foreach ($meashures_rs as $msRow)
                   {   $meashuresDBRecord = new \App\Models\eNaryad\Measure();
   ...
   // !!! повернення до редагування 1 частини наряду
    public function reedit($order_id)  {
           ...
           if (isset($this->naryadRecord['brigade_m'])) {
            $brigade_txt = Brigade_Member::find(explode(",",$this->naryadRecord['brigade_m']));
           }
           $engineers_txt='';
...
public function reedit4($order_id, Request $request)  {
     ...
     *
     * @param  \App\Models\eNaryad\Naryad   $naryad
     * @return \Illuminate\Http\Response
     */
...
//        $record->delete();
    ...
    public function pdf(Naryad $naryad)
    {
        ...
        if (isset($this->naryadRecord['brig_m_ch'])) {
         $brigade_txt = Brigade_Member::find(explode(",",$this->naryadRecord['brig_m_ch']));
        }
        $engineers_txt='';
